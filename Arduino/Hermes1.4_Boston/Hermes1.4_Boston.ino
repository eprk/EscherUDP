...
void check_photo() {
  // BOSTON: no idle time! Just stay high until the opticalTTL is white.
  // Just "digitalize" the phototransistor signal. 
...
//    }
    ...
    digitalWrite(TTL1, LOW); //report stimulus end
  }
}
...
void assignParsedStr(String substr){
  ...
  // GAB for Boston: turn on TTL2 (switch on/off the IR LED for pupillometry)
  if(sub1.equals("TTL2")){ //start cmos
    if(sub2.equals("1")){
      digitalWrite(TTL2, HIGH); //turn on the IR LED
    }
    if(sub2.equals("0")){
      digitalWrite(TTL2, LOW); //turn off the IR LED
    }
  }
