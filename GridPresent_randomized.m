function timestamps = GridPresent_randomized(app,ParameterVector)
    
    % Every stimulus is different in spatial freq, orientation and time
    % post stimulation, so sF, Angle and PostG will be vectors of n
    % elements!
...
%     TrialStartTime = timOffset+(0:n-1)*totPeriod; % start of the baseline
...
%                 end
        ...
        timestamps(i,2) = vbl; % The last vbl acquired.
...
% amplitude inc, with an offset of Glumi:
        for i=1:n
            grating(:,i) = Glumi + inc.*cos(2.*pi.*f(i).*x);
        end
    else
