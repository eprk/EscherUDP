function [subfunc]=pco_camera_mysubfunction()
...
%subfunc
 ...
 fh_get_timestamp_plus = @get_timestamp_plus;
 ...
 subfunc=struct('fh_start_camera',fh_start_camera,...
                ...
                'fh_get_timestamp_plus',fh_get_timestamp_plus,...
...
function [txt,time,ImgCounter]=get_timestamp_plus(ima,act_align,bitpix)
% This is a custom all-in-one function that takes an image as input to
% obtain three outputs:
% 1) A character vector containing the timestamp plus the counter of images 
% (basically everything that is printed on the image itself).
% 2) A datetime variable corresponding to the timestamp of that image.
% 3) A numeric scalar that is the image counter of this image.
% This function is based on both print_timestamp_t and print_timestamp_s
% (which is called by print_timestamp_t).
    
% This part is based on print_timestamp_t
    ts=double(ima(1,1:14));
    if (act_align==0)
        ts=fix(ts/(2^double(16-bitpix)));   
    end
    
% This part is based on print_timestamp_s
% Each pixel contains two digits in a byte (4 bits per digit).

% First we take the image counter
    ImgCounter = 10000000*fix(ts(1)/16) + 1000000*bitand(ts(1),15) +...
    100000*fix(ts(2)/16) + 10000*bitand(ts(2),15) +...
    1000*fix(ts(3)/16) + 100*bitand(ts(3),15) +...
    10*fix(ts(4)/16) + bitand(ts(4),15);
    
% Then we take the datetime from the image.
    time = datetime(1000*fix(ts(5)/16) + 100*bitand(ts(5),15)+... % year, part 1
        10*fix(ts(6)/16) + bitand(ts(6),15),... % year, part 2
        10*fix(ts(7)/16) + bitand(ts(7),15),... % month
        10*fix(ts(8)/16) + bitand(ts(8),15),... % day
        10*fix(ts(9)/16) + bitand(ts(9),15),... % hour
        10*fix(ts(10)/16) + bitand(ts(10),15),... % minutes
        10*fix(ts(11)/16) + bitand(ts(11),15),... % seconds
        100*fix(ts(12)/16) + 10*bitand(ts(12),15)+... % microseconds, digit 1 and 2
        fix(ts(13)/16) + 0.1*bitand(ts(13),15)+... % microseconds, digit 3 and 4
        0.01*(ts(14)/16) + 0.001*bitand(ts(14),15)); % microseconds, digit 5 and 6
    
% Starting from the datetime, we generate a character vector with the time
% and the image counter.
    txt = [num2str(ImgCounter,'%08d'),' ',datestr(time)];
end
