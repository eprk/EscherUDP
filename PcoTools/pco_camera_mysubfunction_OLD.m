function [subfunc]=pco_camera_mysubfunction_OLD()
...
% An image counter can be created. 8 digits.
ImgCounter = 10000000*fix(ts(1,1)/16) + 1000000*bitand(ts(1,1),15) +...
    100000*fix(ts(2,1)/16) + 10000*bitand(ts(2,1),15) +...
    1000*fix(ts(3,1)/16) + 100*bitand(ts(3,1),15) +...
    10*fix(ts(4,1)/16) + bitand(ts(4,1),15);

time = datetime(1000*fix(ts(5,1)/16) + 100*bitand(ts(5,1),15)+... % year, part 1
    10*fix(ts(6,1)/16) + bitand(ts(6,1),15),... % year, part 2
    10*fix(ts(7,1)/16) + bitand(ts(7,1),15),... % month
    10*fix(ts(8,1)/16) + bitand(ts(8,1),15),... % day
    10*fix(ts(9,1)/16) + bitand(ts(9,1),15),... % hour
    10*fix(ts(10,1)/16) + bitand(ts(10,1),15),... % minutes
    10*fix(ts(11,1)/16) + bitand(ts(11,1),15),... % seconds
    100*fix(ts(12,1)/16) + 10*bitand(ts(12,1),15)+... % microseconds, digit 1 and 2
    fix(ts(13,1)/16) + 0.1*bitand(ts(13,1),15)+... % microseconds, digit 3 and 4
    0.01*(ts(14,1)/16) + 0.001*bitand(ts(14,1),15)); % microseconds, digit 5 and 6

b = [num2str(ImgCounter,'%08d'),' ',datestr(time)];
end
