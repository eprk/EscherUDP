...
if CalibrationFlag
    % convert luminance values into 0-1 values
...
% Next lines: convert 0-1 values into 8-bit (or 16-bit) values.
...
% "ArrangeRects" is a function provided by Psychtoolbox.
% cellRects contains the rectangle coordinates. Two screen buffers are
...
if ard_flag
    ...
    cellColor2Off = [cellColor2On, cast([0; 0; 0],app.ScreenBitDepth)];
    cellColor1On = [cellColor1On, cast([app.white; app.white; app.white],app.ScreenBitDepth)];
    cellColor2On = [cellColor2On, cast([app.white; app.white; app.white],app.ScreenBitDepth)];
...
% Prepare the rectangle (entire screen) with equiluminant gray.
Screen('FillRect', app.w, StandbyColor, BaselineRect)


if interrupted
    % display the standby screen as soon as possible. This is done calling
    % Screen without the 3rd argument, that is the time of the flip.
    disp('STOPPED BY KEYBOARD')
    t_end=Screen('Flip', app.w);
else
    % wait until the last half period is over
