...
BackgroundColor = cast([back_lumi; back_lumi; back_lumi].*colormask, app.ScreenBitDepth);
%     BaselineColor = cast(basel_lumi, app.ScreenBitDepth);
%     BaselineScreen = app.screenRect;


if ard_flag && PcoWhileStimFlag % prepare the additional rectangle for optical synch signal
    
%     BaselineColorOff = cast([[basel_lumi;basel_lumi;basel_lumi],[0;0;0]],...
%         app.ScreenBitDepth);
    BaselineColorOff = cast( [ones(3,1).*basel_lumi.*colormask, zeros(3,1)],...
        app.ScreenBitDepth);
    BaselineColor    = cast( [ones(3,1).*basel_lumi.*colormask, ones(3,1).*app.white], ...
        app.ScreenBitDepth);
    ...
    StandbyColor = cast([ones(3,1).*Standby_lumi.*colormask, zeros(3,1)], ...
        app.ScreenBitDepth);
    StandbyScreen = [app.screenRect; app.HermesRect]';
    
elseif ard_flag
    BaselineColor     = cast([ones(3,1).* basel_lumi.* colormask, zeros(3,1)], ...
        app.ScreenBitDepth);
    BaselineColor_ttl = cast([ones(3,1).* basel_lumi.* colormask, ones(3,1).*app.white], ...
        app.ScreenBitDepth);
    BaselineScreen = [app.screenRect; app.HermesRect]';
    ...
    BaselineColor = cast(ones(3,1).* basel_lumi.* colormask, app.ScreenBitDepth);
    BaselineScreen = app.screenRect;
    
    StandbyColor = cast(ones(3,1).* Standby_lumi.* colormask, app.ScreenBitDepth);
    StandbyScreen = app.screenRect;
end
...
if dotsSize_pxl_new~=dotsSize_pxl
    warning('Actual dot size (%.4G pxl) differs from the selected size (%.4G pxl)',...
        dotsSize_pxl_new,dotsSize_pxl)
end
...
while i <= n && ~interrupted
    ...
    while (vbl < timEnd(i)) && ~interrupted
        ...
        % draw the dots
        Screen('DrawDots', app.w, xymatrix, dotsSize_pxl, ...
                cast( ones(3,1).* colormask.* dots_lumi, app.ScreenBitDepth), center, 1);
...
% End of the stimulation loop

Screen('FillRect',app.w,StandbyColor,StandbyScreen)
if interrupted
