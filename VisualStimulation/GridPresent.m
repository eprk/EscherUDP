...
% Potentially, every stimulus is different in contrast, spatial frequency,
% orientation and time post stimulation, so: inc, sF, Angle and PostG
% can be either scalars or vectors of n elements.
% However, for compatibility with UDP communication, ParameterVector is
% a cell array of scalars.
% To properly parse it, we need to know the number of inc, sF, Angle and 
% PostG in advance. The variables "n_contrasts", "n_sF", "n_angles" and "n_PostG" are here for
% that purpose. E.g: if there are 10 contrast values, n_contrasts will be
% equal to 10. The following 10 cells of ParameterVector will contain the
% contrast values. The following code parses ParameterVector step-by-step.
...
% Read the first group of parameters. They are scalars and their position is
% fixed.
[Glumi,gridType,dS,MaskFlag,GaussSize,Bt,n,dark_bsl, ...
    PreG,GridT] = ParameterVector{1:10};

% How many "PostG" values are there? Read all of them
current_index = 11;
n_PostG       = ParameterVector{current_index};
next_index    = current_index + 1 + n_PostG;
PostG         = cat(1,ParameterVector{ current_index+1 : next_index-1});

% Same for "inc"
current_index = next_index;
n_contrasts   = ParameterVector{current_index};
next_index    = current_index + 1 + n_contrasts;
inc           = cat(1,ParameterVector{ current_index+1 : next_index-1});

% Same for "sF"
current_index = next_index;
n_sF          = ParameterVector{current_index};
next_index    = current_index + 1 + n_sF;
sF            = cat(1,ParameterVector{ current_index+1 : next_index-1});

% Same for "Angle"
current_index = next_index;
n_Angle       = ParameterVector{current_index};
next_index    = current_index + 1 + n_Angle;
Angle         = cat(1,ParameterVector{ current_index+1 : next_index-1});

% The last parameters are also scalar.
current_index = next_index;
[PcoWhileStimFlag,OneScreenFlag,CalibrationFlag,...
	ard_flag,baseline_ttl,oculusFlag,optDtrTime] = ParameterVector{current_index:end};
...
% Prepare event timepoints. Note that they all have n+1 alaments. This is
% intended: TrialStartTime(n+1) is used to restore stanby luminance after
% the end of the stimulation, and it makes sure the last "post-grid" baseline
% lasts as it is suppose to do.
TrialStartTime = timOffset+cumsum([0;totPeriod]); % start of the baseline
BaseDtrEndTime = TrialStartTime + optDtrTime; % end of the optical DTR of the baseline
timStart = timOffset+cumsum([0;totPeriod])+PreG; % start of the grid
dtrendtime = timOffset+cumsum([0;totPeriod])'+PreG+optDtrTime; % end of optical DTR for Hermes
timEnd = timOffset+cumsum([0;totPeriod])'+PreG+GridT; % end of the grid
