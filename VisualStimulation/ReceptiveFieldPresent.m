...
[Blumi,Slumi,Bt,St,p,n,nrows,ncols] = ParameterVector{1:8};

% number of squares
n_spots       = nrows*ncols;
indices         = cat(1,ParameterVector{ 9 : 9 + n_spots });

% last parameters
[PcoWhileStimFlag,OneScreenFlag,CalibrationFlag,...
    ard_flag,~,oculusFlag,~] = ParameterVector{9 + n_spots +1 : end};

if PcoWhileStimFlag
    warning('PcoWhileStimFlag=true not supported for "ReceptiveFieldPresent" yet!')
end
if oculusFlag
    warning('oculusFlag=true not supported for "ReceptiveFieldPresent" yet!')
end
...
if ard_flag
    BaselineColor = cast( [Blumi.*colormask, zeros(3,1)],...
        app.ScreenBitDepth);
    StimColor = cast( [ Blumi.*colormask, ...
                        ones(3,1).*app.white,...
                        ones(3,1).*Slumi],...
                    app.ScreenBitDepth);
   StandbyColor = cast( [StandbyLumi.*colormask, zeros(3,1)], ...
    ...
    BaselineColor = cast( Blumi.*colormask, app.ScreenBitDepth);
    StimColor = cast( [Blumi.*colormask,Slumi.*colormask], app.ScreenBitDepth);
    StandbyColor = cast( StandbyLumi.*colormask, app.ScreenBitDepth);
...
if interrupted
    ...
    timestamps(end) = Screen('Flip', app.w, timOffset + n*n_spots*p);
end
