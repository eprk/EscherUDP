...
GratingStruct = CreateGratings(app, Blumi, Slumi, Glumi, f,...
    RectangleSizePx, Angle, oculusFlag, ard_flag, colormask);
...
if ard_flag % prepare the additional rectangle for optical synch signal
    BaselineColorDtrOff = cast([ Glumi.*colormask, zeros(3,1)], ...
        app.ScreenBitDepth);
    BaselineColorDtrOn  = cast([ Glumi.*colormask, ones(3,1)*app.white], ...
        app.ScreenBitDepth);
    BaselineScreen = [app.screenRect; app.HermesRect]';
    StandbyColor = cast([StandbyLumi.*colormask, zeros(3,1)], ...
        app.ScreenBitDepth);
else
    BaselineColorDtrOff = cast( Glumi.*colormask, app.ScreenBitDepth);
...
if Angle==0 || Angle==180
    ...
    %         x-dimension
    TravelDistance = sR(3) + RectangleSizePx;
    ...
    if Angle==0
        ...
        DriftingStartCoorX = sR(3)+1 + round(RectangleSizePx/2);
        dSPxX = dSPx;
    else
        % The vertical bar moves from right to left.
        DriftingStartCoorX = 0 - round(RectangleSizePx/2);
        dSPxX = -dSPx;
    end
    ...
    % y-dimension
    TravelDistance = sR(4) + RectangleSizePx;
    ...
    if Angle==90
        ...
        DriftingStartCoorY = 0 - round(RectangleSizePx/2);
        dSPxY = -dSPx;
    else
        % The horizontal bar moves from top to bottom.
        DriftingStartCoorY = sR(4)+1 + round(RectangleSizePx/2);
        dSPxY = dSPx;
    end
end
...
%             This vector contains 3 timestamps for each period, one for
...
while i <= n && ~interrupted
    ...
    % Animation loop:
    while vbl < TravelEndTime(i) && ~interrupted
...
% Now the masks
% The function is 0 at center of the screen (transparent) and goes up to 
% 255 (opaque).
% rectmask is an opaque mask with a transparent rectangle at the center. 
% At each side of the transparent rectangle are two opaque regions as large
% as the largest dimension of the screen.
% rectmask has dimensions screenHeight x 2screenWidth x RGBA.
if Angle == 0 || Angle == 180
    rectmask = zeros(sR(4), 2*sR(3), 4);
    rectmask(:,:,1:3) = Glumi; % everything is set to gray.
    rectmask(:,:,4) = 255; % everything is set to opaque.
    % Then, only the vertical rectangle is set to transparent (alpha = 0).
    rectmask(:, sR(3)+1-RectangleSizePx/2 : sR(3)+RectangleSizePx/2, 4) = 0;
else
    rectmask = zeros(2*sR(4) + RectangleSizePx, sR(3), 2);
    rectmask(:,:,1:3) = Glumi; % everything is set to gray.
    rectmask(:,:,4) = 255; % everything is set to opaque.
    % Then, only the horizontal rectangle is set to transparent (alpha = 0).
    rectmask(sR(4)+1-RectangleSizePx/2 : sR(4)+RectangleSizePx/2, :, 4) = 0;
end
...
% Only if the optical DTR is checked, the alpha channel is made opaque only
% where the optical DTR should appear. Everywhere else it is just
% transparent.
if ard_flag
...
% Add the RGB dimension as 3rd dimention to accomodate the colormask.
GratingOne = cast( repmat(GratingOne,1,1,3) .* reshape(colormask,1,1,3), app.ScreenBitDepth);
% GratingTwo = cast( GratingTwo, app.ScreenBitDepth);
GratingTwo = cast( repmat(GratingTwo,1,1,3) .* reshape(colormask,1,1,3), app.ScreenBitDepth);
rectmask = cast( rectmask .* reshape( [colormask; 1], 1, 1, 4), app.ScreenBitDepth);
mask_on = cast(mask_on, app.ScreenBitDepth); % this mask 3rd dimension is just GA (grayscale and alpha)
mask_off = cast(mask_off, app.ScreenBitDepth); % this mask 3rd dimension is just GA (grayscale and alpha)
